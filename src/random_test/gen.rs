...
use crate::parse::analyze_format;
...
/// Walk the format tree collecting the `sum_limit` denominators: the first
/// `TestCases.count` (= `T`) and every jagged `Array`'s length→count mapping.
pub(crate) fn collect_sum_denominators(spec: &ResolvedSpec) -> Denominators {
    ...
    Denominators {
        ...
        // Persisted yml generated by the parser has at most one outer TestCases block.
