...
**戦略は1テストケースにつき1つ選ばれ、入力全体に適用される。** ただし効果は変数の種類によって異なる（例: `ArrayMonoInc` はスカラーはランダム生成、配列のみ単調増加）。

| 戦略 | スカラー変数への効果 | 配列変数への効果 | 検出する問題パターン |
|------|------------------|---------------|-------------------|
| AllMax | 全変数を上限値 | 全要素を上限値 | 大入力でのTLE・バッファオーバーフロー |
| AllMin | 全変数を下限値 | 全要素を下限値 | 0・1要素の境界処理 |
| SmallSize(1) | サイズ変数=1、他はランダム | サイズ=1の配列 | 要素数1の境界処理 |
| SmallSize(2) | サイズ変数=2、他はランダム | サイズ=2の配列 | 要素数2の境界処理 |
| SmallSize(3) | サイズ変数=3、他はランダム | サイズ=3の配列 | 小さい配列での動作 |
| ZeroCorner | lo<0<hi の変数を0、他はランダム | ランダム | 符号変化・ゼロ除算・0を含む境界 |
| SumMaxSingle | T=1・内側変数=sum上限、他は最大 | 最大サイズの配列 | sum制約下での最大単一ケース |
| ArrayMonoInc | ランダム | 単調増加列 | ソート済み入力・二分探索の境界 |
| ArrayMonoDec | ランダム | 単調減少列 | 逆順ソート済み入力 |
| ArrayAllMax | ランダム | 全要素を上限値 | 全同値（最大）・整数オーバーフロー |
| ArrayAllMin | ランダム | 全要素を下限値 | 全同値（最小） |
| ArrayAllSame | ランダム | 全要素を同じランダム値 | 全同値の一般ケース・重複処理 |
| ArrayAltMaxMin | ランダム | max,min,max,min…と交互 | 交互パターン・奇偶インデックス処理 |
| ArrayMountain | ランダム | 増加→減少（山型） | 単峰性を仮定したアルゴリズム |
| ArrayOneMaxRestMin | ランダム | 1要素のみ上限、残りは下限 | 外れ値・孤立した最大値 |
| Random | 完全ランダム | 完全ランダム | 一般ケース |

**未実装のコーナーケース:**
- 文字列入力のコーナーケース（`'a'`のみ、`'z'`のみ等）❌
- グリッドのコーナーケース（`#`のみ、`.`のみ等）❌
- 境界値周辺（max-1、min+1）❌
