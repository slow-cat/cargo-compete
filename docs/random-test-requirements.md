...
| SmallSize(k) | サイズ変数=k、他ランダム | サイズ=kの配列、要素ランダム | ランダム長・ランダム文字 | k行・ランダム文字 | 小さい配列での動作 |
| ZeroCorner | lo<0<hi の変数→0、他ランダム | ランダム | ランダム | ランダム | 符号変化・ゼロ除算・0境界 |
| SumMaxSingle | T=1・内側変数=sum上限、他最大 | 上限サイズ×1回分 | 最大長・末尾文字 | 最大行数×1回分 | sum制約下での最大単一ケース |
| ArrayMonoInc | ランダム | 単調増加列（行列は行ごとに増加） | ランダム | 行が先頭→末尾文字に昇順変化（例: `aaa`→`mmm`→`zzz`） | ソート済み入力・二分探索の境界 |
| ArrayMonoDec | ランダム | 単調減少列（行列は行ごとに減少） | ランダム | 行が末尾→先頭文字に降順変化 | 逆順ソート済み入力 |
| ArrayAllMax | ランダム | 全要素=上限値 | ランダム | 全行=末尾文字のみ | 全同値（最大）・オーバーフロー |
| ArrayAllMin | ランダム | 全要素=下限値 | ランダム | 全行=先頭文字のみ | 全同値（最小） |
| ArrayAllSame | ランダム | 全要素=同一ランダム値・全行同一 | ランダム | 全行=同一ランダム文字列 | 全同値・重複処理 |
| ArrayAltMaxMin | ランダム | 上限・下限を交互 | ランダム | ランダム | 交互パターン・奇偶インデックス処理 |
| ArrayMountain | ランダム | 増加→減少（山型） | ランダム | ランダム | 単峰性を仮定したアルゴリズム |
| ArrayOneMaxRestMin | ランダム | 中央1要素=上限、残り=下限 | ランダム | ランダム | 外れ値・孤立した最大値 |
