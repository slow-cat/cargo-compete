...
| SmallSize(k) ※k=1,2,3の3件 | サイズ変数(テストケース数含む)=k.clamp(lo,hi)（loがVar未解決時はhi以下にクランプのみ）、他ランダム | ランダム | ランダム | 左記に影響を受ける |   |   | ⭕️ |   |   | ⭕️ | 小さい配列での動作 |
...
例外的にArrayAltMaxMinは各行依存で一松模様にする。整数の場合も。可変長二次元配列(Jageed)なら各行の1つ目が交互になっている状態。3次元なら最終的な出力が市松模様に見えるように(つまり問題文での縦方向=2次元目*3次元目を1次元として扱って市松模様)
...
yml の `vars` で `type: Chars` かつ `values` が未指定の場合、ランダムテスト生成時は
**英大文字 + 英小文字の 52 文字（A–Z, a–z）** をデフォルト charset として使用する。

| 条件 | charset |
|------|---------|
| `values: [a, b, c, ...]` が指定されている | そのリストのみ |
| `values` 未指定（`type: Chars` のみ） | A–Z + a–z（52 文字） |
