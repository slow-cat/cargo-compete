...
| 戦略 | 対象 | 生成内容 | 検出する問題パターン |
|------|------|---------|-------------------|
| AllMax | 全スカラー | 全変数を上限値 | 大入力でのTLE・バッファオーバーフロー |
| AllMin | 全スカラー | 全変数を下限値 | 0・1要素の境界処理 |
| SmallSize(1) | サイズ変数 | サイズ=1、他はランダム | 要素数1の境界処理 |
| SmallSize(2) | サイズ変数 | サイズ=2、他はランダム | 要素数2の境界処理 |
| SmallSize(3) | サイズ変数 | サイズ=3、他はランダム | 小さい配列での動作 |
| ZeroCorner | 全スカラー | lo<0<hi の変数を0に設定 | 符号変化・ゼロ除算・0を含む境界 |
| SumMaxSingle | 外側T・内側変数 | T=1、内側変数=sum上限値 | sum制約下での最大単一ケース |
| ArrayMonoInc | 配列要素 | 単調増加列 | ソート済み入力・二分探索の境界 |
| ArrayMonoDec | 配列要素 | 単調減少列 | 逆順ソート済み入力 |
| ArrayAllMax | 配列要素 | 全要素を最大値 | 全同値（最大）・整数オーバーフロー |
| ArrayAllMin | 配列要素 | 全要素を最小値 | 全同値（最小） |
| ArrayAllSame | 配列要素 | 全要素を同じランダム値 | 全同値の一般ケース・重複処理 |
| ArrayAltMaxMin | 配列要素 | max,min,max,min…と交互 | 交互パターン・奇偶インデックス処理 |
| ArrayMountain | 配列要素 | 増加→減少（山型） | 単峰性を仮定したアルゴリズム |
| ArrayOneMaxRestMin | 配列要素 | 1要素のみ最大値、残りは最小値 | 外れ値・孤立した最大値 |
| Random | 全変数 | 完全ランダム | 一般ケース |
